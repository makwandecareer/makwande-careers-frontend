...
set "BACKUP=%PROJECT%\phase12-5-backup-%DATESTAMP%-%TIMESTAMP%"

echo Installing Phase 12.5...
...
copy /Y "%PACKAGE%components\cv-builder\ExperienceInterviewEngine.tsx" "%PROJECT%\components\cv-builder\ExperienceInterviewEngine.tsx" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%components\cv-builder\ExperienceInterviewEngine.module.css" "%PROJECT%\components\cv-builder\ExperienceInterviewEngine.module.css" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%lib\experience-interview.ts" "%PROJECT%\lib\experience-interview.ts" >nul
if errorlevel 1 goto :error

echo Phase 12.5 installed successfully.
echo.
echo Next:
echo cd /d %PROJECT%
echo npm run build
