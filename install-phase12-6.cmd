...
set "BACKUP=%PROJECT%\phase12-6-backup-%DATESTAMP%-%TIMESTAMP%"

echo Installing Phase 12.6...
...
copy /Y "%PACKAGE%components\cv-builder\TechnicalDeepDiveEngine.tsx" "%PROJECT%\components\cv-builder\TechnicalDeepDiveEngine.tsx" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%components\cv-builder\TechnicalDeepDiveEngine.module.css" "%PROJECT%\components\cv-builder\TechnicalDeepDiveEngine.module.css" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%lib\technical-deep-dive.ts" "%PROJECT%\lib\technical-deep-dive.ts" >nul
if errorlevel 1 goto :error

echo Phase 12.6 installed successfully.
echo.
echo Next:
echo cd /d %PROJECT%
echo npm run build
