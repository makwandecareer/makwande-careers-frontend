...
set "BACKUP=%PROJECT%\phase21-backup-%DATESTAMP%-%TIMESTAMP%"
...
copy /Y "%PACKAGE%components\cv-builder\CVIntakeRevampCentre.tsx" "%PROJECT%\components\cv-builder\CVIntakeRevampCentre.tsx" >nul || goto :error
copy /Y "%PACKAGE%components\cv-builder\CVIntakeRevampCentre.module.css" "%PROJECT%\components\cv-builder\CVIntakeRevampCentre.module.css" >nul || goto :error
copy /Y "%PACKAGE%lib\cv-intake-revamp.ts" "%PROJECT%\lib\cv-intake-revamp.ts" >nul || goto :error

echo Phase 21 installed successfully.
echo Backup available at %BACKUP%
pause
exit /b 0

:error
echo ERROR: Phase 21 installation failed.
echo Backup available at %BACKUP%
pause
exit /b 1
