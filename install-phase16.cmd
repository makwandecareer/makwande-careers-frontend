...
set "BACKUP=%PROJECT%\phase16-backup-%DATESTAMP%-%TIMESTAMP%"

echo Installing Phase 16...
...
copy /Y "%PACKAGE%components\cv-builder\RecruiterEmployerPortal.tsx" "%PROJECT%\components\cv-builder\RecruiterEmployerPortal.tsx" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%components\cv-builder\RecruiterEmployerPortal.module.css" "%PROJECT%\components\cv-builder\RecruiterEmployerPortal.module.css" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%lib\recruiter-employer-portal.ts" "%PROJECT%\lib\recruiter-employer-portal.ts" >nul
if errorlevel 1 goto :error

echo Phase 16 installed successfully.
echo.
echo Next:
echo cd /d %PROJECT%
echo npm run build
