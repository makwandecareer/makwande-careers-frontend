...
set "BACKUP=%PROJECT%\phase15-backup-%DATESTAMP%-%TIMESTAMP%"

echo Installing Phase 15...
...
copy /Y "%PACKAGE%components\cv-builder\ApplicationCommandCentre.tsx" "%PROJECT%\components\cv-builder\ApplicationCommandCentre.tsx" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%components\cv-builder\ApplicationCommandCentre.module.css" "%PROJECT%\components\cv-builder\ApplicationCommandCentre.module.css" >nul
if errorlevel 1 goto :error

copy /Y "%PACKAGE%lib\application-command-centre.ts" "%PROJECT%\lib\application-command-centre.ts" >nul
if errorlevel 1 goto :error

echo Phase 15 installed successfully.
echo.
echo Next:
echo cd /d %PROJECT%
echo npm run build
